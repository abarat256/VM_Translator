################################################### Arithmetic/Logic Binary Instruction ###########################################################

def arith_logic_inst_binary(command,label_val):
    newline=""
    if command == "add":
        newline += "@SP\n" 
        newline += "AM=M-1\n"
        ...
        newline = "Invalid Syntax"
    return newline, label_val
	
################################################### Arithmetic/Logic Unary Instruction ###########################################################
	
def arith_logic_inst_unary(command):
    newline=""
    if command == "neg":
        newline += "@SP\n" 
        newline += "A=M-1\n" 
        ...
        for line in file_descriptor:
            ...
            if (line[:2] != "//") and (line[:2] != "/*") and (line[:2] !="/*") and (line != '\n'):
                ...
                if command == "push" or command == "pop" :
                    newline= push_pop_inst(line)
                elif command in  arith_logic_table_binary :
                    newline, label_val= arith_logic_inst_binary(command,label_val)
                elif command in arith_logic_unary:
                    newline= arith_logic_inst_unary(command)
                else :
                    print("Invalid Syntax")
                    break
                if newline == "Invalid Syntax":
                    break
                else:
